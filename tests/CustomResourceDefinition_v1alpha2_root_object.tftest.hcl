run "without_properties" {
  command = plan
  module {
    source = "./CustomResourceDefinition/v1alpha2/root_object/"
  }
...
run "with_properties" {
  ...
  assert {
    condition     = output.schema.version == "v1"
    error_message = "Não retornou um outout.schema indicando o uso de um root_object v1"
